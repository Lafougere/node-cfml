...
function removeCFMLComments(str){
	var openCount = 0, out = "", start = 0, match
	reComment.lastIndex = 0
	while (match = reComment.exec(str)) {
		if (match[0].indexOf('--->') > -1) {
			if (openCount === 1) {
				// replace comment block by newlines so that the line numbers still correspond
				out += str.substring(start, match.index + match[0].length).replace(/[^\n]/gm, '')
			...
			if (openCount === 0) {
				out += str.substr(start, match.index - start)
				start = match.index
			}
			openCount++;
		}
...
exports.removeCFMLComments = removeCFMLComments
